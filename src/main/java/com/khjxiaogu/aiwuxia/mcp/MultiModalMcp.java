...
public class MultiModalMcp {
	public static MCPTools create(ObjectStorageProvider tos,AISession session) {
		...
		tools.register(new ToolData.Builder("image_recognition", "使用多模态模型查看图片并返回图片描述。该功能仅负责描述图片内容，注意需要甄别描述中的内容是否合理，是否符合人物设定，人物是否应该知道对应内容。描述中的分析仅供参考，你需要独立判断是否正确。")
				.putParam("picture_id", "72位16进制的图片id，只包含图片id本身，不得包含任何其他内容").tool((data) -> {
					...
					if (mtch.find()) {
						...
						return recognizeImage(tos,id,session);
	...
	public static String recognizeImage(ObjectStorageProvider tos,String id,AISession session) {
		if (tos.exists(id)) {
			Builder builder = AIRequest.builder(session).taskType(TaskType.STORY)
			...
			try {
				...
				output.addUsageListener(session::addUsage);
